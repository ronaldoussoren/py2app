...
class TestBasicApp(unittest.TestCase):
    ...
    def start_app(self):
        ...
        path = os.path.join(self.app_dir, "dist/quot.app/Contents/MacOS/quot")
    ...
    def test_simple_imports(self):
        ...
        p.stdin.write(
            (
                "print(%r in sys.path)\n"
                % (os.path.join(self.app_dir, "dist/quot.app/Contents/Resources"),)
            ).encode("latin1")
        )
        p.stdin.flush()
    ...
    def test_zip_contents(self):
        ...
        zfpath = os.path.join(
            dirpath, "Resources/lib/python%d%d.zip" % (sys.version_info[:2])
        )
        if not os.path.exists(zfpath):
            zfpath = os.path.join(
                dirpath,
                "Resources/lib/python%d.%d/site-packages.zip" % (sys.version_info[:2]),
            )
        if not os.path.exists(zfpath):
